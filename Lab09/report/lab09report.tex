...
\begin{itemize}
  \item Input Resistor ($R_1$): $\SI{50.842}{\kilo\ohm} \pm \SI{0.001}{\kilo\ohm}$
  \item Feedback Resistor ($R_2$): $\SI{50.850}{\kilo\ohm} \pm \SI{0.001}{\kilo\ohm}$
  \item Feedback Capacitor ($C_1$): $\SI{50.0}{\nano\farad} \pm \SI{0.1}{\nano\farad}$
\end{itemize}

The gain magnitude and cutoff frequency were calculated from measured components using
\[ |G| = \frac{R_2}{R_1}, \qquad f_c = \frac{1}{2\pi R_2 C_1}. \]
Using first-order uncertainty propagation,
\[ u_G = |G|\sqrt{\left(\frac{u_{R_1}}{R_1}\right)^2 + \left(\frac{u_{R_2}}{R_2}\right)^2}, \]
\[ u_{f_c} = f_c\sqrt{\left(\frac{u_{R_2}}{R_2}\right)^2 + \left(\frac{u_{C_1}}{C_1}\right)^2}. \]
From these relations we obtain $|G| = 1.000157 \pm 0.000028$ and $f_c = \SI{62.60 \pm 0.13}{\hertz}$.
...
\subsection{Component and Calculated Uncertainty}
Table~\ref{tab:uncertainty_summary} summarizes the component measurements and propagated uncertainty for gain and cutoff frequency, as required in Task 1(b)--(c).

\begin{table}[H]
  \centering
  \caption{Measured Components and Propagated Uncertainty}
  \label{tab:uncertainty_summary}
  \begin{tabular}{ll}
    \toprule
    \textbf{Quantity} & \textbf{Value} \\
    \midrule
    $R_1$ & $\SI{50.842 \pm 0.001}{\kilo\ohm}$ \\
    $R_2$ & $\SI{50.850 \pm 0.001}{\kilo\ohm}$ \\
    $C_1$ & $\SI{50.0 \pm 0.1}{\nano\farad}$ \\
    $|G| = R_2/R_1$ & $1.000157 \pm 0.000028$ \\
    $f_c = 1/(2\pi R_2 C_1)$ & $\SI{62.60 \pm 0.13}{\hertz}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lSSS}
    ...
    Method 3 (Curve fit)        & 3.7360                   & 0.1172               & 42.60 \\
    \bottomrule
  \end{tabular}
\end{table}

Method 3 (exponential curve fit) exhibited the lowest standard deviation (\SI{0.1172}{\milli\second}), indicating it is the most repeatable and reliable approach. Method 1 (95\% threshold) showed moderate scatter (std dev = \SI{0.2294}{\milli\second}), while Method 2 (99.3\% threshold) was severely compromised by outliers, yielding a standard deviation of \SI{3.4343}{\milli\second}---nearly 29 times larger than Method 3.

The measured steady-state gain from the step data was $V_0/V_{in} = 1.6188 \pm 0.0600$ (mean $\pm$ standard deviation across 30 runs, with $V_{in}=\SI{1}{\volt}$).

\subsection{Time-Domain Waveforms}
Fig.~\ref{fig:step0to6tau} displays the raw step response from three representative trials (Runs 1, 15, and 30) over the full 0 to 6$\tau$ window, using the curve-fit time constant (\SI{3.736}{\milli\second}) to set the time scale. All three traces follow the expected first-order exponential rise with no overshoot or ringing, consistent with the passive low-pass filter topology.

Fig.~\ref{fig:step4to6tau} provides a zoomed view of the settling region (4$\tau$ to 6$\tau$). The vertical dashed line marks $t = 5\tau$, at which point the system has reached 99.3\% of steady state. The marker labels on the updated plot show the actual magnitudes at $t=5\tau$ for the three runs (Run 1: \SI{1.766}{\volt}, Run 15: \SI{1.606}{\volt}, Run 30: \SI{1.536}{\volt}), directly satisfying the handout requirement for a clear mark of response magnitude.
...
\begin{figure}[H]
  ...
  \caption{Zoomed view of the settling transient (4$\tau$ to 6$\tau$). The dashed line at $t = 5\tau$ marks the theoretical 99.3\% settling point, and the text labels report each run's measured voltage at that instant.}
...
\textbf{Method 1 vs. Method 3:} Method 1 (95\% threshold) and Method 3 (curve fit) both converge to similar mean values: \SI{3.7191}{\milli\second} and \SI{3.7360}{\milli\second}, respectively. The curve-fit approach is superior because it uses the entire rising edge (not just a single sample), making it less vulnerable to noise and digitization artifacts. Method 3's standard deviation (\SI{0.1172}{\milli\second}) is also about half that of Method 1 (\SI{0.2294}{\milli\second}), confirming that fitting is a more statistically robust extraction technique.
...
\subsection{Unity-Gain Requirement and Measured Gain}

The handout specified a target gain magnitude of 1. From measured resistor values, the intended gain is very close to unity:
\[ |G| = \frac{R_2}{R_1} = 1.000157 \pm 0.000028. \]
However, the measured steady-state gain from the step data was significantly higher:
\[ \frac{V_0}{V_{in}} = 1.6188 \pm 0.0600, \qquad (V_{in}=\SI{1}{\volt}). \]
This indicates a systematic setup issue rather than random noise. Since resistor-ratio gain error is only about 0.016\%, the likely causes are (i) wiring/node selection error (measuring a different node than the intended filter output), (ii) grounding/reference mismatch between DAQ and analog circuit, or (iii) DAQ analog input scaling/calibration offset. A pure resistor tolerance explanation cannot produce a 62\% gain increase. In future runs, this should be verified by directly measuring both DAQ output and filter output with a benchtop oscilloscope/DMM while the DAQ is running.
...
\begin{align*}
  ...
  \tau_{\text{measured (Method 3)}} &= \SI{3.7360}{\milli\second} \\
  \text{Discrepancy} &= 3.7360 - 2.5425 = \SI{1.1935}{\milli\second} \approx 46.9\%
\end{align*}
...
Across 30 trials, Method 3 yielded a standard deviation of only \SI{0.1172}{\milli\second}, or approximately 3.1\% coefficient of variation relative to the mean. This exceptional reproducibility confirms that:
...
The small variance also validates our confidence in the curve-fit time constant as the canonical estimate of system response. Any further analysis or design iterations should use $\tau = \SI{3.736}{\milli\second}$ as the empirical baseline.
...
The step response provides direct information about settling time. The $5\tau$ rule of thumb states that a first-order system reaches 99.3\% of steady state after $5\tau$. Given our measured $\tau = \SI{3.736}{\milli\second}$, the settling time is:
\[ t_{\text{settling}} = 5\tau = 5 (3.736) = \SI{18.68}{\milli\second} \]
...
Moreover, the relationship $\tau = 1/(2\pi f_c)$ confirmed by our frequency response (Lab 8) and transient analysis (this lab) demonstrates that the circuit is truly a first-order low-pass filter. A system characterized by a \SI{62.6}{\hertz} cutoff from measured components must exhibit a \SI{2.54}{\milli\second} time constant in the time domain. Our measured \SI{3.736}{\milli\second} suggests either component tolerance issues or unmodeled parasitic effects—a discrepancy worth resolving in next-iteration design.

\section{Conclusion}
This experiment successfully characterized the transient response of the active low-pass filter via step-input analysis. Across 30 independent trials, we extracted the time constant using three methods and demonstrated that exponential curve fitting yields the most statistically robust estimate: $\tau = \SI{3.7360 \pm 0.1172}{\milli\second}$. This measured time constant translates to a cutoff frequency of $f_c \approx \SI{42.60}{\hertz}$, a 31.9\% reduction from the component-derived value of \SI{62.60}{\hertz}. Uncertainty requirements were addressed explicitly: $|G| = 1.000157 \pm 0.000028$ and $f_c = \SI{62.60 \pm 0.13}{\hertz}$ from propagated component uncertainty. The measured step-based gain ($1.6188 \pm 0.0600$) indicates a probable setup or scaling issue that should be verified with direct scope/DMM checks in a repeat experiment. The absence of overshoot and the excellent run-to-run repeatability confirm first-order behavior suitable for anti-aliasing applications where predictable transient response is required.
